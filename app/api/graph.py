...
def _ask_discovery_question(st: SessionState) -> str:
    qa_json = _json.dumps(st.qa, ensure_ascii=False, indent=2)
    examples_text = ""
...
def _refine_hypotheses(st: SessionState, correction_text: str) -> List[str]:
    ...
    qa_json = _json.dumps(st.qa, ensure_ascii=False, indent=2)
    examples_text = ""
    hypos_json = _json.dumps(st.last_hypotheses, ensure_ascii=False, indent=2)
    ...
    try:
        ...
        refined_arr = _json.loads(refine_raw)
