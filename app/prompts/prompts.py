...
def build_generate_followups_prompt(
    ...
    return (
        "Ты — консультант по бизнес-аналитике. Сформируй уточняющие вопросы для дальнейшей диагностики.\n"
...
def build_select_hypothesis_prompt(
    ...
    return (
        "Ты — аналитик. На входе ответы КМ и список типовых гипотез (по одной в строке).\n"
        "ЗАДАЧА: выбрать ОДНУ гипотезу из списка, которая ЛУЧШЕ ВСЕГО СООТВЕТСТВУЕТ фактам.\n"
        ...
        "Учитывай тренды выручки/прибыли, стагнацию/падение рынка, текучесть, ликвидность, действия конкурентов.\n"
...
def build_select_subhypothesis_prompt(
    ...
    facts_block = f"\nФакты:\n{facts_summary}\n" if facts_summary else ""
    return (
        "Ты — аналитик. На входе ответы КМ и список побочных гипотез для выбранной основной.\n"
