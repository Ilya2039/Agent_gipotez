...
"""
Промпты для агента стратегии: вопросы, решения следующего шага, гипотезы, вопросы для встречи.
"""
...
def build_is_unknown_answer_prompt(answer: str) -> str:
    return (
        "Определи, означает ли ответ отсутствие знания/данных по вопросу.\n"
        "Ответь одним словом без пояснений: 'да' — если по смыслу это 'не знаю/нет данных/затрудняюсь/не уверен/хз/n/a/-, —, .../skip', иначе 'нет'.\n"
        "Примеры (строго следуй им):\n"
        "не знаю -> да\n"
        "Не знаю -> да\n"
        "затрудняюсь ответить -> да\n"
        "— -> да\n"
        "n/a -> да\n"
        "да -> нет\n"
        "нет -> нет\n"
...
def build_example_answer_prompt(question: str, answers_json: str, dialog_json: str = "") -> str:
    ...
    return (
        ...
        "Без лишних вводных и дисклеймеров. По делу, по-русски.\n"
        f"Вопрос: {question}\nОтветы КМ (JSON):\n{answers_json}\n{dialog_block}"
