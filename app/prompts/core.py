...
def build_is_unknown_answer_prompt(answer: str) -> str:
    return (
        "Определи, означает ли ответ пользователя отсутствие знания/данных.\n"
        "Считай unknown=true, если по смыслу это: 'не знаю', 'нет данных/информации', 'затрудняюсь ответить', 'сложно сказать', 'без понятия', 'не помню', 'не уверен', 'хз', 'n/a', '-', '—', '...'.\n"
        "Игнорируй регистр, вежливые формулировки и пунктуацию; фокусируйся на смысле.\n"
